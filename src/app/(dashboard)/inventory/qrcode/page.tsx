...
import { Plus, Trash2, Printer, RotateCcw, QrCode } from "lucide-react";
...
export default function QRCodePage() {
  ...
  const addToPrintList = () => {
    ...
    if (!batch) return;
    if (printList.find((item) => item.barcode === batch.barcode)) return;
    setPrintList((prev) => [
      ...prev,
      {
  ...
  const handleGenerate = async () => {
    try {
      await apiFetch("/products/qrcode/generate", {
        method: "POST",
        body: JSON.stringify({
          ...
          <div className="bg-white rounded-xl border border-border p-5 mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="secondary" onClick={handlePrint}>
                <Printer size={16} className="mr-2" /> Print QR Code
              </Button>
            </div>
          </div>
        </>
