...
export default function ExpenseCategoriesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = await apiFetch<ExpenseCategory[]>(`/finance/expense-categories?${params}`);
      setCategories(data);
    } catch {
      setCategories([]);
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editItem) {
        await apiFetch(`/finance/expense-categories/${editItem.id}`, { method: "PATCH", body: JSON.stringify(body) });
      } else {
        await apiFetch("/finance/expense-categories", { method: "POST", body: JSON.stringify(body) });
      }
      setModalOpen(false);
      fetchData();
    } catch (e: any) {
  ...
  const columns = [
    { key: "#", label: "#", className: "w-12", render: (_: ExpenseCategory, i: number) => i + 1 },
    { key: "name", label: "Name", render: (c: ExpenseCategory) => <span className="font-medium">{c.name}</span> },
    { key: "status", label: "Status", render: (c: ExpenseCategory) => <StatusBadge status={c.status} /> },
    {
      key: "actions",
      label: "Actions",
