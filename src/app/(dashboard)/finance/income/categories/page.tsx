...
export default function IncomeCategoriesPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await apiFetch<unknown>("/finance/income-categories");
      const list = Array.isArray(res) ? (res as IncomeCategory[]) : [];
      setRows(
        list.map((r) => ({
          ...r,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editItem) {
        await apiFetch(`/finance/income-categories/${editItem.id}`, {
          method: "PATCH",
          body: JSON.stringify(body),
        });
      } else {
        await apiFetch("/finance/income-categories", {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
  ...
  const columns = [
    { key: "#", label: "#", className: "w-12", render: (_: IncomeCategory, i: number) => (page - 1) * PAGE_SIZE + i + 1 },
    { key: "name", label: "Name", render: (r: IncomeCategory) => <span className="font-medium">{r.name}</span> },
    { key: "status", label: "Status", render: (r: IncomeCategory) => <StatusBadge status={r.status || "inactive"} /> },
    {
      key: "actions",
      label: "Actions",
      render: (r: IncomeCategory) => (
        <TableRowActions>
  ...
  return (
    <div className="w-full min-w-0 space-y-5 pb-8 pt-1 sm:space-y-6 sm:pb-10 sm:pt-2">
      <InventoryListPageHeader icon={Tags} title="Income Categories" description="Manage income categories in one place.">
