...
interface ProductQtyReportResponse {
  summary?: {
...
export default function ProductQuantityReportPage() {
  ...
  const [report, setReport] = useState<ProductQtyReportResponse | null>(null);
  ...
  const fetchReport = useCallback(
    async (targetPage: number, limitOverride?: number) => {
      ...
      try {
        ...
        const res = await apiFetch<ProductQtyReportResponse>(`/reports/product-quantity?${q.toString()}`);
        setReport(res);
    ...
    () => [
      {
        ...
        label: "#",
        className: "w-12",
        render: (_r: QtyLine, index: number) => (
      ...
      },
      {
        key: "productName",
        label: "Product",
        render: (r: QtyLine) => <span className="font-semibold text-foreground">{r.productName}</span>,
      },
      {
        key: "variant",
        label: "Variant",
      ...
      },
      {
        key: "branchName",
        label: "Branch",
        render: (r: QtyLine) => <span className="text-muted-foreground">{r.branchName}</span>,
      },
      {
        key: "categoryName",
        label: "Category",
        render: (r: QtyLine) => <span>{r.categoryName ?? "—"}</span>,
      },
      {
        key: "brandName",
        label: "Brand",
        render: (r: QtyLine) => <span className="text-muted-foreground">{r.brandName ?? "—"}</span>,
      },
      {
        key: "availableQty",
        label: "Available",
        className: "text-right",
        render: (r: QtyLine) => <span className="tabular-nums font-medium">{r.availableQty}</span>,
      },
      {
        key: "soldQty",
        label: "Sold",
        className: "text-right",
        render: (r: QtyLine) => <span className="tabular-nums text-muted-foreground">{r.soldQty}</span>,
      },
      {
        key: "sellingPrice",
        label: "Price",
      ...
      },
      {
        ...
        className: "text-right",
        render: (r: QtyLine) => (
          <span className="tabular-nums font-semibold">{formatPrice(Number(r.totalValue))}</span>
        ),
      },
    ],
    [meta.page, limit],
  ...
  const getReportRows = () =>
    list.map((row, i) => [
      ...
      formatPrice(Number(row.totalValue)),
    ]);
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>Product Quantity Report</title>
      ...
      <p style="font-size:14px;color:#444">Current stock (active products, non-archived)</p>
  ...
  const handleExportCsv = () => {
    ...
    a.href = url;
    a.download = `product-quantity-report-${today}.csv`;
    a.click();
  ...
  return (
    <div className="w-full min-w-0 space-y-5 pb-8 pt-1 sm:space-y-6 sm:pb-10 sm:pt-2">
      <InventoryListPageHeader
        ...
        description="Per branch / variant stock: available quantity, historical sold from batches, and line value."
      >
        <Button
          type="button"
          variant="outline"
      ...
      <section className={INVENTORY_CARD_SHELL}>
        ...
        {generated && report ? (
          <div className="grid grid-cols-1 gap-3 border-t border-border/60 px-5 py-4 sm:grid-cols-2 lg:grid-cols-5 sm:px-6 md:px-7">
            <StatCard title="Distinct products" value={String(s?.distinctProducts ?? "—")} icon={Package} />
            <StatCard title="Store lines" value={String(s?.totalLines ?? meta.total)} icon={Hash} />
            <StatCard title="Available qty" value={String(s?.totalAvailableQty ?? "—")} icon={Hash} />
            <StatCard title="Sold (batches)" value={String(s?.totalSoldQty ?? "—")} icon={TrendingUp} />
            <StatCard title="Stock value" value={formatPrice(s?.totalValue ?? 0)} icon={DollarSign} />
          </div>
        ) : null}

        {!generated && !loading ? (
          <div className="border-t border-border/60 px-5 py-10 text-center text-sm text-muted-foreground sm:px-6">
            Choose filters and Generate to load quantity lines.
          </div>
        ) : null}
        ...
        {generated && !loading && list.length === 0 ? (
          <div className="border-t border-border/60 px-5 py-12 text-center text-sm text-muted-foreground sm:px-6">
            <Boxes className="mx-auto mb-3 h-12 w-12 opacity-40" aria-hidden />
            <p className="m-0 font-medium">No lines match the selected filters.</p>
          </div>
        ) : null}
