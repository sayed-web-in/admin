...
export default function CustomerDueReportPage() {
  ...
  const [report, setReport] = useState<CustomerDueReport | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await apiFetch<CustomerDueReport>(`/reports/customer-due?${params}`);
      setReport(res);
    } catch {
      setReport(null);
    } finally {
  ...
  const handlePrint = () => {
    ...
    if (!win) return;
    win.document.write(`<html><head><title>Customer Due Report</title>
      ...
      <div class="summary">
        ...
        <span><strong>Customers with Due:</strong> ${report.customersWithDue}</span>
      </div>
  ...
  const columns = [
    { key: "customer", label: "Customer", render: (r: CustomerDueRow) => <span className="font-medium">{r.customer}</span> },
    { key: "phone", label: "Phone", render: (r: CustomerDueRow) => r.phone || "—" },
    { key: "totalPurchase", label: "Total Purchase", render: (r: CustomerDueRow) => formatPrice(r.totalPurchase) },
    { key: "paid", label: "Paid", render: (r: CustomerDueRow) => <span className="text-green-600">{formatPrice(r.paid)}</span> },
    { key: "due", label: "Due", render: (r: CustomerDueRow) => <span className="text-red-600 font-semibold">{formatPrice(r.due)}</span> },
  ];
  ...
  return (
    <div>
      <PageHeader
        ...
        description="View outstanding customer dues"
      ...
      {report && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <StatCard title="Customers with Due" value={report.customersWithDue} icon={Users} />
        </div>
      )}

      <div ref={tableRef}>
        <DataTable columns={columns} data={report?.customers || []} loading={loading} />
      </div>
    </div>
  );
}
