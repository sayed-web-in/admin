...
export default function ExpenseReportPage() {
  ...
  useEffect(() => {
    void apiFetch<unknown>("/finance/expense-categories")
      .then((c) => {
  ...
  const fetchReport = useCallback(
    async (targetPage: number, limitOverride?: number) => {
      ...
      try {
        ...
        const res = await apiFetch<ExpenseReportResponse>(`/reports/expense?${q.toString()}`);
        setReport(res);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (r: FinanceLine) => (
          <span className="font-semibold tabular-nums">{formatPrice(Number(r.amount))}</span>
        ),
      },
      {
        key: "account",
  ...
  const handleExportCsv = () => {
    ...
    a.href = url;
    a.download = `expense-report-${today}.csv`;
    a.click();
  ...
  return (
    <div className="w-full min-w-0 space-y-5 pb-8 pt-1 sm:space-y-6 sm:pb-10 sm:pt-2">
      <InventoryListPageHeader
        ...
        description="Active expenses by date range with category, branch, account, and paginated lines (seller-style columns)."
      >
        <Button
          type="button"
          variant="outline"
      ...
      <section className={INVENTORY_CARD_SHELL}>
        ...
        {generated && report ? (
          <div className="grid grid-cols-1 gap-3 border-t border-border/60 px-5 py-4 sm:grid-cols-2 lg:grid-cols-4 sm:px-6 md:px-7">
            <StatCard title="Total expenses" value={formatPrice(report.totalExpenses)} icon={DollarSign} />
        ...
        {!generated && !loading ? (
          <div className="border-t border-border/60 px-5 py-10 text-center text-sm text-muted-foreground sm:px-6">
            Set the period and filters, then Generate to load expense lines.
          </div>
        ) : null}
        ...
        {generated && !loading && list.length === 0 ? (
          <div className="border-t border-border/60 px-5 py-12 text-center text-sm text-muted-foreground sm:px-6">
            <TrendingDown className="mx-auto mb-3 h-12 w-12 opacity-40" aria-hidden />
            <p className="m-0 font-medium">No expenses found for the selected filters.</p>
          </div>
        ) : null}
