...
function totalProductQty(row: PurchaseRow): number {
  if (typeof row.totalProducts === "number") return row.totalProducts;
  return row.items?.reduce((s, i) => s + (i.quantity || 0), 0) ?? 0;
}
...
export default function PurchaseReportPage() {
  ...
  const fetchReport = useCallback(
    async (targetPage: number, limitOverride?: number) => {
      ...
      try {
        ...
        const res = await apiFetch<PurchaseReportResponse>(`/reports/purchase?${q.toString()}`);
        setReport(res);
    ...
    [dateFrom, dateTo, branchId, paymentStatus, limit],
  );
    ...
    () => [
      {
        ...
        render: (r: PurchaseRow) => (
          <div>
            <span className="font-medium text-foreground">{r.branch?.name ?? "—"}</span>
            {r.referenceNo ? (
              <span className="mt-0.5 block text-xs text-muted-foreground">{r.referenceNo}</span>
            ) : null}
          </div>
        ),
      },
      {
        ...
        render: (r: PurchaseRow) => (
          <span className="tabular-nums text-muted-foreground">{totalProductQty(r)}</span>
        ),
      },
      {
        key: "responsible",
        label: "Responsible",
        render: (r: PurchaseRow) => (
          <span className="text-muted-foreground">{r.responsiblePerson?.trim() || "—"}</span>
        ),
      },
      {
        key: "grandTotal",
      ...
      },
      {
        ...
        render: (r: PurchaseRow) => (
          <span
            className={`tabular-nums ${Number(r.dueAmount) > 0 ? "font-medium text-rose-700 dark:text-rose-400" : "text-muted-foreground"}`}
          >
            {formatPrice(Number(r.dueAmount))}
          </span>
        ),
      },
      {
  ...
  const handleExportCsv = () => {
    ...
    a.href = url;
    a.download = `purchase-report-${today}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const grandSummary =
    summary?.totalGrand ?? report?.totalGrand ?? report?.totalAmount ?? 0;
  const paidSummary = summary?.totalPaid ?? report?.totalPaid ?? 0;
  const dueSummary = summary?.totalDue ?? report?.totalDue ?? 0;
  ...
  return (
    <div className="w-full min-w-0 space-y-5 pb-8 pt-1 sm:space-y-6 sm:pb-10 sm:pt-2">
      <InventoryListPageHeader
        ...
        description="Purchases by date, branch, and payment status — same columns as seller-admin (branch, products, responsible, amounts, payment, date)."
      >
        <Button
          type="button"
          variant="outline"
      ...
      {generated && summary ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard title="Total purchases" value={summary.totalPurchases} icon={BarChart3} />
          <StatCard title="Grand total" value={formatPrice(grandSummary)} icon={DollarSign} />
          <StatCard title="Total paid" value={formatPrice(paidSummary)} icon={CreditCard} />
          <StatCard title="Total due" value={formatPrice(dueSummary)} icon={CreditCard} />
        </div>
      ) : null}
      ...
      <div className="space-y-4">
        ...
        {generated && !loading && list.length === 0 ? (
          <div className={`${INVENTORY_CARD_SHELL} p-12 text-center text-muted-foreground`}>
            <ShoppingBag className="mx-auto mb-3 h-12 w-12 opacity-40" aria-hidden />
            <p className="m-0 font-medium">No purchases found for the selected filters.</p>
          </div>
        ) : null}
              ...
              {list.map((row) => (
                <div key={row.id} className={`${INVENTORY_CARD_SHELL} p-4`}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-foreground">{row.branch?.name ?? "—"}</p>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span>Products {totalProductQty(row)}</span>
                    <span>·</span>
