...
type QuickAddCategoryModalProps = {
  open: boolean;
...
export function QuickAddCategoryModal({
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const created = await apiFetch<{ id: number; name: string }>("/categories", {
        method: "POST",
        body: JSON.stringify({
          name: name.trim(),
  ...
  return (
    <Modal
      ...
      title="Add category"
      description="Create a category without leaving the product form."
